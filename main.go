	...
	for {
		...
		if songToDownload != nil {
			...
			for attempt := 0; attempt < 3; attempt++ {
				if attempt > 0 {
					log.Printf("[DOWNLOADER] Retry attempt %d for \"%s\"", attempt+1, songToDownload.Title)
					time.Sleep(time.Duration(attempt) * 5 * time.Second) // Wait longer between retries
				}
				...
				switch attempt {
				case 0:
					// First attempt: HLS format 91 (this works around YouTube restrictions)
					downloadCmd = exec.CommandContext(ctx, "nice", "-n", "19", "yt-dlp", "-f", "91", "--extract-audio", "--audio-format", "opus", "-o", filenameTemplate, songToDownload.URL)
				case 1:
					// Second attempt: worst audio format
					downloadCmd = exec.CommandContext(ctx, "nice", "-n", "19", "yt-dlp", "-f", "worstaudio/worst", "--extract-audio", "--audio-format", "opus", "-o", filenameTemplate, songToDownload.URL)
				case 2:
					// Third attempt: extract audio with opus format
					downloadCmd = exec.CommandContext(ctx, "nice", "-n", "19", "yt-dlp", "--extract-audio", "--audio-format", "opus", "--audio-quality", "0", "-o", filenameTemplate, songToDownload.URL)
				}
			...
			if err != nil {
				...
				b.twitchClient.Say(b.config.Channel, fmt.Sprintf("Error: YouTube downloads are currently broken due to YouTube restrictions. Try again later or use a direct URL.", songToDownload.Title))
				...
				// If download is successful, construct the filename directly.
				actualFilename := filepath.Join(b.downloadDir, songToDownload.ID+".opus")
...
func getYoutubeInfo(query string) (*YTDLPInfo, error) {
	...
	if info.Type == "playlist" {
		// Use webpage_url for playlist entries (not the direct video URL)
		for i := range info.Entries {
			if info.Entries[i].WebpageURL != "" {
				info.Entries[i].URL = info.Entries[i].WebpageURL
			} else if info.Entries[i].URL == "" {
