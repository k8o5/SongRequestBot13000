...
func (b *Bot) handleAdd(message twitch.PrivateMessage) {
	...
	go func() {
		...
		var songsToAdd []Song
		if isSearch {
			// If it's a search, we treat it as a single song request.
			// If yt-dlp returns a playlist for the search, we take the first entry.
			...
			// If it's a URL, it could be a playlist or a single video.
...
func (b *Bot) playerLoop() {
	for {
		...
		if songToPlay != nil {
			...
			b.nowPlayingID = "" // Clear after song finishes
		}
...
func getYoutubeInfo(query string) (*YTDLPInfo, error) {
	...
	// If yt-dlp returns a playlist, we pass it through without validation
	// to avoid checking every single song.
	...
	// For single videos, perform validation.
	if info.IsLive {
